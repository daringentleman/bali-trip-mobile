(()=>{
  ...
  window.renderWeek=function(){
    ...
    const col=100/7;
    for(const e of [...events].sort((a,b)=>whenMs(a)-whenMs(b))){const di=DAYS.findIndex(d=>d[0]===e.date);if(di<0)continue;const top=mins(e.start)*WEEK_PPM,ht=Math.max(30,duration(e)*WEEK_PPM);html+=`<div class="eventblock ${esc(e.kind||'activity')}" data-id="${e.id}" style="top:${top}px;height:${ht}px;left:calc(${di*col}% + 2px);width:calc(${col}% - 4px)"><div class="et">${esc(e.start)}</div><div class="en">${esc(shortTitle(e.activity))}</div></div>`}
    const wb=document.getElementById('weekBody');wb.style.height=(24*60*WEEK_PPM+160)+'px';wb.innerHTML=html;
  ...
  window.weekDown=function(ev){
    ...
    downPoint={x:ev.clientX,y:ev.clientY,id:e.id,el,pointerId:ev.pointerId};
    longPressTimer=setTimeout(()=>startWeekDrag(ev,el,e),500);
    const move=pe=>{if(!downPoint)return;if(Math.hypot(pe.clientX-downPoint.x,pe.clientY-downPoint.y)>9){clearTimeout(longPressTimer);cleanup()}};
  ...
  window.startWeekDrag=function(ev,el,e){
    if(!downPoint)return;navigator.vibrate?.(25);el.classList.add('drag-active','dragging');el.dataset.dragged='1';
    const rect=el.getBoundingClientRect(),g=el.cloneNode(true);g.classList.add('dragghost');g.classList.remove('drag-active','dragging');g.style.left=rect.left+'px';g.style.top=rect.top+'px';g.style.width=rect.width+'px';g.style.height=rect.height+'px';document.body.appendChild(g);
    ...
    const mv=pe=>{if(!drag)return;pe.preventDefault();drag.lastX=pe.clientX;drag.lastY=pe.clientY;updateWeekDrag(pe.clientX,pe.clientY);edgeAutoScroll()};
    ...
    document.addEventListener('pointermove',mv,{passive:false});document.addEventListener('pointerup',up,{once:true});document.addEventListener('pointercancel',up,{once:true});downPoint=null;
  };
  ...
  window.edgeAutoScroll=function(){
    if(edgeRaf)return;
    const step=()=>{edgeRaf=0;if(!drag)return;const sc=document.getElementById('weekCalendar'),r=sc.getBoundingClientRect(),y=drag.lastY;let delta=0;if(y<r.top+54)delta=-16;else if(y>r.bottom-54)delta=16;if(delta){sc.scrollTop=Math.max(0,sc.scrollTop+delta);updateWeekDrag(drag.lastX,drag.lastY);edgeRaf=requestAnimationFrame(step)}};
    edgeRaf=requestAnimationFrame(step);
  };
  window.finishWeekDrag=function(){if(edgeRaf){cancelAnimationFrame(edgeRaf);edgeRaf=0}const e=events.find(x=>x.id===drag.id);e.date=drag.targetDate;e.start=timeFromMins(drag.targetMin);e.end=timeFromMins(Math.min(1439,drag.targetMin+drag.dur));drag.el.classList.remove('drag-active','dragging');drag.ghost.remove();drag.dateTag.remove();drag=null;saveEvents();renderAll();showToast('行程已移動')};
