(()=>{
  ...
  function renderBlock(e,di,col){
    ...
    return `<div class="eventblock ${e.major?'major':''}" data-id="${e.id}" style="top:${top}px;height:${ht}px;left:calc(${di*col}% + 2px);width:calc(${col}% - 4px)"><div class="et">${esc(e.start)}</div><div class="en">${esc(shortTitle(e.activity))}</div></div>`;
  }
  ...
  window.renderWeek=function(){
    ...
    const wb=document.getElementById('weekBody');wb.style.height=(24*60*WEEK_PPM+200)+'px';wb.innerHTML=html;
  ...
  function touchStart(ev){
    ...
    hold={type:'touch',el,e,id:t.identifier,sx:t.clientX,sy:t.clientY,lastX:t.clientX,lastY:t.clientY,phase:'holding'};
  ...
  function touchMove(ev){
    ...
    if(hold.phase==='holding'&&Math.hypot(dx,dy)>7){
  ...
  function mouseStart(ev){
    ...
    const move=me=>{
      if(!hold)return;
      hold.lastX=me.clientX;hold.lastY=me.clientY;
      if(hold.phase==='holding'&&Math.hypot(me.clientX-hold.sx,me.clientY-hold.sy)>7){clearTimeout(hold.timer);hold.phase='cancelled'}
      if(hold.phase==='dragging'&&dragState){dragState.lastX=me.clientX;dragState.lastY=me.clientY;moveGhost(me.clientX,me.clientY);previewTarget(me.clientX,me.clientY);edgeScroll()}
    };
  ...
  function beginDrag(el,e,x,y){
    ...
    navigator.vibrate?.(20);hold.phase='dragging';el.classList.add('drag-active','dragging');el.dataset.dragged='1';
    const rect=el.getBoundingClientRect(),g=el.cloneNode(true);g.classList.add('dragghost');g.classList.remove('drag-active','dragging');
  ...
  function moveGhost(x,y){
    if(!dragState)return;
    dragState.ghost.style.left=(x-dragState.grabOffsetX)+'px';
    dragState.ghost.style.top=(y-dragState.grabOffsetY)+'px';
  }
  ...
  function edgeScroll(){
    ...
    const step=()=>{
      ...
      if(delta){sc.scrollTop=Math.max(0,Math.min(sc.scrollHeight-sc.clientHeight,sc.scrollTop+delta));previewTarget(dragState.lastX,dragState.lastY);edgeRaf=requestAnimationFrame(step)}
    };
  ...
  function finishDrag(){
    ...
    dragState.el.classList.remove('drag-active','dragging');dragState.ghost.remove();dragState.dateTag.remove();dragState=null;saveEvents();renderAll();
  }
